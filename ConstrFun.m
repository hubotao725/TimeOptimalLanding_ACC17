...
g(7*N-4:8*N-5)=-(y(1:N,1)+0.25*cos(theta(1:N,1))).*tan(0.3*Ts*(0:N-1)')+(z(1:N,1)-0.25*sin(theta(1:N,1))-.1*sin(Ts*(0:N-1)'));
g(8*N-4:9*N-5)=-(y(1:N,1)-0.25*cos(theta(1:N,1))).*tan(0.3*Ts*(0:N-1)')+(z(1:N,1)+0.25*sin(theta(1:N,1))-.1*sin(Ts*(0:N-1)'));
g(9*N-4)=y(1,1);
g(9*N-3)=y(N,1)-.25*cos(0.3*b(8*N+1));
g(9*N-2)=y(1,2);
g(9*N-1)=y(N,2);
g(9*N)=z(1,1)-5;
g(9*N+1)=z(N,1)-.1*sin(b(8*N+1))-.25*sin(0.3*b(8*N+1));
g(9*N+2)=z(1,2);
g(9*N+3)=z(N,2)-.1*cos(b(8*N+1)); 
g(9*N+4)=theta(1,1);
g(9*N+5)=theta(N,1)+.3*b(8*N+1);
g(9*N+6)=b(8*N)+.3;
end    
